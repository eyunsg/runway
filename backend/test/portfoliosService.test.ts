...
describe('PortfolioService - 포트폴리오 생성 테스트', () => {
  ...
  describe('getPortfolioDetailService', () => {
    ...
    it('Soft-deleted된 포트폴리오 상세 조회 시 NOT_FOUND 에러를 던진다', async () => {
      // 레포지토리에서 deleted_at IS NULL 필터링 결과로 데이터가 조회되지 않은 상황 모킹
      (getPortfolioDetailRepo as jest.Mock).mockResolvedValue(null);

      await expect(getPortfolioDetailService(mockUserId, mockPortfolioId)).rejects.toThrow(
        'NOT_FOUND'
      );
    });
  });
  ...
  describe('updatePortfolioService', () => {
    ...
    it('Soft-deleted된 포트폴리오 수정 시 NOT_FOUND 에러를 던진다', async () => {
      // 레포지토리에서 deleted_at IS NULL 조건 때문에 업데이트할 대상을 찾지 못한 상황 모킹
      (updatePortfolioRepo as jest.Mock).mockResolvedValue(false);
  ...
  describe('deletePortfolioService', () => {
    const mockPortfolioId = 'port-delete-999';

    it('정상적인 ID와 소유권이 확인되면 성공적으로 삭제된다', async () => {
      // 1. 리포지토리 성공 모킹 (true 반환)
      (deletePortfolioRepo as jest.Mock).mockResolvedValue(true);

      // 2. 서비스 호출 및 예외 미발생 검증
      await expect(deletePortfolioService(mockUserId, mockPortfolioId)).resolves.not.toThrow();

      // 3. 리포지토리에 올바른 인자가 전달되었는지 확인
      expect(deletePortfolioRepo).toHaveBeenCalledWith(mockUserId, mockPortfolioId);
      expect(deletePortfolioRepo).toHaveBeenCalledTimes(1);
    });

    it('삭제할 포트폴리오가 없거나 권한이 없으면 NOT_FOUND 에러를 던진다', async () => {
      // 1. 리포지토리 실패 모킹 (false 반환)
      (deletePortfolioRepo as jest.Mock).mockResolvedValue(false);

      // 2. 서비스 호출 시 NOT_FOUND 예외 발생 검증
      await expect(deletePortfolioService(mockUserId, 'wrong-id')).rejects.toThrow(
        'NOT_FOUND: 삭제할 포트폴리오를 찾을 수 없거나 권한이 없습니다.'
      );
    });

    it('리포지토리에서 데이터베이스 에러 발생 시 에러를 상위로 전파한다', async () => {
      // 1. 리포지토리 예외 발생 모킹
      (deletePortfolioRepo as jest.Mock).mockRejectedValue(new Error('DB_CONNECTION_LOST'));

      // 2. 서비스가 에러를 가로채지 않고 그대로 전파하는지 확인
      await expect(deletePortfolioService(mockUserId, mockPortfolioId)).rejects.toThrow(
        'DB_CONNECTION_LOST'
      );
    });
  });
