...
Deno.test('Profile Service - 성공적으로 프로필을 조회하고 응답 객체를 반환해야 함', async () => {
  ...
  // [중요] any 대신 Record를 사용하여 불필요한 필드 부재 검증
  const data = result.data as unknown as Record<string, unknown>;
  const resultKeys = Object.keys(data || {});

  // 1. 모델 필드 및 전체 개수 검증
  assertEquals(data?.createdAt, undefined, '결과 데이터에 모델 필드가 섞이면 안 됩니다.');
  assertEquals(resultKeys.length, 2, '필드는 email, displayName 딱 2개여야 합니다.');

  // 2. 팀장님 지시사항(통계 정보 제거)을 명시적으로 한 번 더 검증
  assertEquals(
